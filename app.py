...
def index():
    ...
    global dataRaw
    global dataRandom
    global dataStratified
    global dataReducedRandom
    global dataReducedStratified

    #The current request method is available by using the method attribute
    if request.method == 'POST':
        # KMeans: k <- 3
        if request.form['data'] == 'elbow':
            data = getElbow(dataRaw)
        else:
        ...
        data = dataRaw[['wfood','income']]
        data = data.rename(columns={'wfood':'x', 'income': 'y'})
        chart_data = data.to_dict(orient='records')
        chart_data = json.dumps(chart_data, indent=2)
        data = {'chart_data': chart_data}
...
if __name__ == "app":
    dataRaw = pd.read_csv(ct.DATAPATH)
    app.run(debug=True)
